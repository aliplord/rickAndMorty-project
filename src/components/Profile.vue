<template>
    <header class="container">
        <nav class="header__nav">
            <div class="nav-left">
                <div class="nav-icons">
                    <router-link :to="`/home`"><svg style="cursor: pointer;" width="50px" height="50px"
            ...
            <div class="main-block">
                <div class="card-hero">
                    <div class="profile-info">
                        <h1>Профиль</h1>
                        <img v-if="photo" :src="photo" alt="Фото профиля" class="profile-photo" />
                        <div v-else class="placeholder-photo">Фото</div>
                        <input id="file-upload" type="file" @change="uploadPhoto" accept="image/*" />
                        <label for="file-upload" class="custom-file-upload">Загрузить фото</label>
                    </div>
                    <div class="profile-details">
                        <label>Имя:</label>
                        <input v-model="name" placeholder="Введите ваше имя" />
                        <label>Описание:</label>
                        <textarea v-model="description" placeholder="Напишите о себе"></textarea>
                        <button @click="saveProfile">Сохранить профиль</button>
                    </div>
                    <div class="profile-favorites">
                        <h3>Избранный персонаж</h3>
                        <div v-if="favCharacter.length > 0">
                            <div v-for="character in favCharacter" :key="character.id">
    ...
    setup() {
        ...
        const saveProfile = () => {
            ...
            if (currentUserIndex === -1) {
                alert("Вы не вошли в систему! лол");
                return;
            }
        ...
        onMounted(() => {
            ...
            photo.value = currentUser.photo || "";

            const favsIds = currentUser.favs || [];
            if (favsIds.length > 0) {
                const fetchPromises = favsIds.map((id) =>
                    fetch(`https://rickandmortyapi.com/api/character/${id}`).then((res) => res.json())
                );

                Promise.all(fetchPromises)
                    .then((character) => {
                        favCharacter.value = character;
                    })
                    .catch((error) => {
                        console.error(error);
                    });
