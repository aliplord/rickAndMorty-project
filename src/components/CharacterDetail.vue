    ...
    <main class="container">
        <div class="main-right" :style="{ width: mainRightWidth }">
            ...
            <div class="main-block">
                <div class="card-hero" v-if="character">
                    ...
                    <div class="card-bottom">
                        ...
                        <div class="card-stuff">
                            <router-link :to="`/character/${Math.max(1, parseInt($route.params.id) - 1)}`"
                                v-if="$route.params.id > 1">
                                <button class="card-slide">PREVIOUS</button>
                            </router-link>
                            <router-link :to="`/character/${parseInt($route.params.id) + 1}`">
                                <button class="card-slide">NEXT</button>
                            </router-link>
                            <button class="card-add" @click="likeCard(character.id)">Add</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    setup() {
        ...
        const fetchCharacterDetails = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `не удалось получить данные для персонажа лол ${characterId}`
                    );
                }
        ...
        const likeCard = (id) => {
            ...
            if (currentUser.likes.includes(id)) {
                alert(`персонаж уже в ваших сохранениях так то `);
            } else {
                currentUser.likes.push(id);
                alert(`персонаж успешно добавлен ура`);
            }
