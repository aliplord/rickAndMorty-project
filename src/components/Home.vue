<template>
        ...
        <nav class="header__nav">
            <div class="nav-left">
                <div class="nav-icons">
                    ...
                    <button class="nav-hide" @click="toggleSidebar" v-html="btnIcon"></button>
                </div>
                <ul>
                    <router-link to="/profile">
                        <li><a href="">{{ $t('profile') }}</a></li>
                    </router-link>
                    <router-link to="/saves">
            ...
            <div class="nav-right">
                <input type="text" placeholder="SEARCH" class="nav-input" v-model="searchQuery" @input="handleSearch" />
                <svg @click="logout" width="50px" height="50px" viewBox="0 0 24 24" fill="none"
    ...
    <main class="container">
        <div class="main-left" :style="{ width: sidebarWidth }">
            <div>
                <h2>{{ $t('filter') }}</h2>
            </div>
            <div class="filtex-box">
                <div class="lang-btns">
                    <button class="lang-btn" @click="setLanguage('en')">English</button>
                    <button class="lang-btn" @click="setLanguage('ru')">Русский</button>
                </div>

                <h3 class="filter-title">{{ $t('salesType') }}</h3>
                <div class="filter-checkboxes">
                    <div>
                        <input type="checkbox" id="Common" name="Common" value="Common" v-model="salesTypeFilter" />
                        <label for="Common">{{ $t('common') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="Rare" name="Rare" value="Rare" v-model="salesTypeFilter" />
                        <label for="Rare">{{ $t('rare') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="Legendary" name="Legendary" value="Legendary"
                            v-model="salesTypeFilter" />
                        <label for="Legendary">{{ $t('legendary') }}</label>
                    </div>
                </div>
                ...
                <div class="filter-checkboxes">
                    <div>
                        <input type="checkbox" id="Male" name="Male" value="Male" v-model="genderFilter" />
                        <label for="Male">{{ $t('male') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="Female" name="Female" value="Female" v-model="genderFilter" />
                        <label for="Female">{{ $t('female') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="Unknown" name="Unknown" value="unknown" v-model="genderFilter" />
                ...
                <div class="filter-checkboxes">
                    <div>
                        <input type="checkbox" id="Alive" name="Alive" value="Alive" v-model="statusFilter" />
                        <label for="Alive">{{ $t('alive') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="Dead" name="Dead" value="Dead" v-model="statusFilter" />
                        <label for="Dead">{{ $t('dead') }}</label>
                    </div>
                    <div>
                        <input type="checkbox" id="unknown-life" name="unknown-life" value="unknown"
                            v-model="statusFilter" />
                        <label for="unknown-life">{{ $t('unknown') }}</label>
                    </div>
                </div>

                <div class="filter-func">
                    <button class="sort-btn" @click="applyFilters">{{ $t('sort') }}</button>
                    <button class="reset-btn" @click="resetFilters">{{ $t('reset') }}</button>
                </div>
            </div>

        </div>
        <div class="main-right" :style="{ width: mainRightWidth }">
            ...
            <h2 class="page_num">
                {{ $t('pageNum') }} {{ currentPage }}
            </h2>
            <div class="main-block">
                <div v-for="character in displayedCharacters" :key="character.id" class="card-hero"
                    :style="rareDetect(character)">
                    ...
                    <div class="card-bottom">
                        ...
                        <div class="card-status">
                            <div :style="{ backgroundColor: colorDetect(character.status) }"></div>
                            <p>{{ $t('statusSM') }} {{ character.status }}</p>
                            <p>{{ $t('species') }} {{ character.species }}</p>
                        </div>
                        <div class="card-stuff">
                            <button @click="likeCard(character.id)" class="card-add">{{ $t('add') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="main-bottom">
                <p class="main-prev main-func" @click="prevPage">{{ $t('previuos') }}</p>
                <input class="input-page" type="number" v-model.number="inputPage" placeholder="1 - 42"
                    @change="jumpToPage" />
                <p class="main-next main-func" @click="nextPage">{{ $t('next') }}</p>
            </div>
        </div>
    </main>
...
export default {
    ...
    setup() {
        ...
        const mainRightWidth = ref("calc(100% - 300px)");
        const btnIcon = ref(`
      <svg style="cursor: pointer;" width="50px" height="50px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M13 9.00003L10.2121 11.7879C10.095 11.9051 10.095 12.095 10.2121 12.2122L13 15M21 17V7.00003C21 4.79089 19.2091 3.00003 17 3.00003H7C4.79086 3.00003 3 4.79089 3 7.00003L3 17C3 19.2092 4.79086 21 7 21H17C19.2091 21 21 19.2092 21 17Z" stroke="#000000" stroke-width="2" stroke-linecap="round" />
      </svg>
    `);
        ...
        const toggleSidebar = () => {
            if (sidebarOpen.value) {
                ...
                mainRightWidth.value = "100%";
                btnIcon.value = `
          <svg style="cursor: pointer;" width="50px" height="50px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11 9.00003L13.7879 11.7879C13.905 11.9051 13.905 12.095 13.7879 12.2122L11 15M3 17V7.00003C3 4.79089 4.79086 3.00003 7 3.00003H17C19.2091 3.00003 21 4.79089 21 7.00003V17C21 19.2092 19.2091 21 17 21H7C4.79086 21 3 19.2092 3 17Z" stroke="#000000" stroke-width="2" stroke-linecap="round" />
          </svg>
        `;
            } else {
                sidebarWidth.value = "300px";
                mainRightWidth.value = "calc(100% - 300px)";
                btnIcon.value = `
          <svg style="cursor: pointer;" width="50px" height="50px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M13 9.00003L10.2121 11.7879C10.095 11.9051 10.095 12.095 10.2121 12.2122L13 15M21 17V7.00003C21 4.79089 19.2091 3.00003 17 3.00003H7C4.79086 3.00003 3 4.79089 3 7.00003L3 17C3 19.2092 4.79086 21 7 21H17C19.2091 21 21 19.2092 21 17Z" stroke="#000000" stroke-width="2" stroke-linecap="round" />
          </svg>
        ...
        const fetchPageCharacters = async (page) => {
            try {
        ...
        const handleSearch = async () => {
            ...
            if (query.trim() === "") {
                ...
                fetchPageCharacters(currentPage.value);
            } else {
                isSearching.value = true;
                if (allCharacters.value.length === 0) {
                    await fetchAllCharacters();
                }
                filteredCharacters.value = allCharacters.value.filter((character) =>
                    character.name.toLowerCase().includes(query)
                );
            }
        };

        const rareDetect = (character) => {
            if (character.episode.length > 5) {
                return "background-color: rgb(172, 165, 59);";
            } else if (character.episode.length > 1 && character.episode.length < 5) {
                return "background-color: rgb(0, 153, 0);";
            } else {
                return "background-color: rgb(85, 79, 79);";
            }
        };

        const colorDetect = (status) => {
            return status === "Alive"
                ? "green"
                : status === "Dead"
                    ? "red"
        ...
        const jumpToPage = () => {
            if (inputPage.value >= 1 && inputPage.value <= maxPages) {
                currentPage.value = inputPage.value;
                fetchPageCharacters(currentPage.value);
            } else {
                alert("введите число от 1 до 42");
            }
        };
        ...
        const likeCard = (id) => {
            ...
            if (currentUser.likes.includes(id)) {
                alert(`персонаж уже в ваших сохранениях`);
            } else {
                currentUser.likes.push(id);
        ...
        const applyFilters = async () => {
            if (allCharacters.value.length === 0) {
                await fetchAllCharacters();
                console.log("All characters after fetching:", allCharacters.value);
            }
            ...
            filteredCharacters.value = allCharacters.value.filter((character) => {
                ...
                const matchesSalesType =
                    salesTypeFilter.value.length === 0 || salesTypeFilter.value.includes(salesType);
                const matchesGender =
                    genderFilter.value.length === 0 || genderFilter.value.includes(character.gender);
                const matchesStatus =
                    statusFilter.value.length === 0 || statusFilter.value.includes(character.status);

                return matchesSalesType && matchesGender && matchesStatus;
            });

            console.log("Filtered characters:", filteredCharacters.value);
        ...
        onMounted(() => fetchPageCharacters(currentPage.value));

        const displayedCharacters = computed(() => {
            if (isFiltered.value) {
                return filteredCharacters.value;
            }
            return isSearching.value ? filteredCharacters.value : characters.value;
        });
...
.sort-btn:focus:not(:focus-visible):not(.focus-visible),
.reset-btn:focus:not(:focus-visible):not(.focus-visible) {
    box-shadow: none;
    outline: none;
}
